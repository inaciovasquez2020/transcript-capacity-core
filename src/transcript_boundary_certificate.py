...
class TranscriptBoundaryCertificate:
    theorem_id: str
    status: str
    required_count: int
    present_count: int
...
def transcript_boundary_certificate(root: Path | str, required: Sequence[str], boundary_text: str) -> TranscriptBoundaryCertificate:
    normalized = _normalize_required(required)
    ...
    return TranscriptBoundaryCertificate(
        theorem_id="TCC-TBC-1",
